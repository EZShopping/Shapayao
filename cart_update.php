...
if(isset($_POST["type"]) && $_POST["type"]=='add')
{
	...
	if ($results) { //we have the product info 
		...
		$new_product = array(array('name'=>$obj->productName, 'code'=>$Product_ID , 'Product Number'=>$product_qty, 'Price'=>$obj->Price));
		...
		if(isset($_SESSION["cart_session"])) //if we have the session
		{
			...
			foreach ($_SESSION["cart_session"] as $cart_itm) //loop through session array
			{
				if($cart_itm["code"] == $Product_ID){ //the item exist in array
					...
					$product[] = array('name'=>$cart_itm["name"], 'code'=>$cart_itm["code"], ''=>$product_qty, 'Price'=>$cart_itm["Price"]);
					$found = true;
				}else{
					//item doesn't exist in the list, just retrive old info and prepare array for session var
					$product[] = array('name'=>$cart_itm["name"], 'code'=>$cart_itm["code"], 'Product Number'=>$cart_itm["Product Number"], 'Price'=>$cart_itm["Price"]);
				}
			}
